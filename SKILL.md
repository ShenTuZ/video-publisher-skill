...
`ready` is computed centrally. Fast mode requires authentication, correct draft identity, completed upload, exact text/topic entities, explicitly requested declarations/settings, no blocking dialog, an enabled final button, and an armed safety guard. Publish timing is always an explicit per-video setting: fast mode must verify the exact immediate/scheduled state and, when scheduled, its exact timestamp before READY or final publication. It skips only untouched default-item gates and the redundant final full-page verification. Strict mode retains every gate and repair. Read `references/platform-common.md` for the gate and blocker contract.
