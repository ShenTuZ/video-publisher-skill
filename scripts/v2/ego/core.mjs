...
async function armFinalPublishGuard() {
  return await js(String.raw`((key, finalSource, finalFlags) => {
    ...
    if (existing?.armed === true && existing.version === 2) {
      return { ok: true, armed: true, version: existing.version, armedAt: existing.armedAt, blockedAttempts: existing.blockedAttempts.length, authorized: existing.authorized === true }
    }
    const finalText = new RegExp(finalSource, finalFlags)
    const compact = value => String(value || '').replace(/\s+/g, ' ').trim()
    const state = { armed: true, version: 2, armedAt: new Date().toISOString(), authorized: false, authorizedAt: null, blockedAttempts: [], allowedAttempts: [] }
    ...
    const guard = event => {
      ...
      if (state.authorized === true) {
        state.allowedAttempts.push({ type: event.type, label: match.label, at: new Date().toISOString() })
        return
      }
...
async function inspectFinalPublishGuard() {
  return await js(String.raw`((key) => {
    ...
    return state?.armed === true && state.version === 2
      ? { armed: true, version: state.version, armedAt: state.armedAt, authorized: state.authorized === true, authorizedAt: state.authorizedAt || null, blockedAttempts: state.blockedAttempts.length, attempts: state.blockedAttempts.slice(-5), allowedAttempts: state.allowedAttempts.slice(-5) }
      : { armed: false, version: state?.version || null, authorized: false, blockedAttempts: state?.blockedAttempts?.length || 0, attempts: state?.blockedAttempts?.slice?.(-5) || [], allowedAttempts: state?.allowedAttempts?.slice?.(-5) || [] }
  })(${JSON.stringify(FINAL_GUARD_KEY)})`);
}

async function authorizeFinalPublishGuard() {
  if (finalPublishAuthorized !== true) return { ok: false, reason: 'final publish phase lacks orchestrator authorization' };
  return await js(String.raw`((key) => {const state=window[key];if(state?.armed!==true||state.version!==2)return {ok:false,reason:'final publish guard is not armed'};if(state.blockedAttempts.length)return {ok:false,reason:'final publish guard recorded a blocked attempt',blockedAttempts:state.blockedAttempts.length};state.authorized=true;state.authorizedAt=new Date().toISOString();return {ok:true,armed:true,authorized:true,authorizedAt:state.authorizedAt}})(${JSON.stringify(FINAL_GUARD_KEY)})`);
}
...
async function emitObservation(observation) {
  ...
  payload.gates ||= {};
  const safetyEvidence = { finalPublishClicked: payload.finalPublishClicked === true, guardArmed: guard.armed === true, blockedAttempts: guard.blockedAttempts || 0, attempts: guard.attempts || [], guardVersion: guard.version || null, authorized: guard.authorized === true, authorizedAt: guard.authorizedAt || null, allowedAttempts: guard.allowedAttempts || [] };
