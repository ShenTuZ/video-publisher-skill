...
test("platform runner rejects a direct publish phase without orchestrator authorization",async()=>{
  const root=await fs.promises.mkdtemp(path.join(os.tmpdir(),"video-publisher-v2-publish-auth-test-"));
  const videoPath=path.join(root,"sample.mp4");
  const packagePath=path.join(root,"package.json");
  const configPath=path.join(root,"config.json");
  await fs.promises.writeFile(videoPath,"test video fixture");
  await fs.promises.writeFile(packagePath,JSON.stringify({videoPath,title:"Publish auth",wechatDescription:"Publish auth\n\n#Test",wechatTags:["Test"],cover:{uploadCustomCover:false}}));
  await fs.promises.writeFile(configPath,JSON.stringify({schemaVersion:2,onboarding:{completed:true},sourceDirectory:root,availablePlatforms:["wechat_channels"],defaultPlatforms:["wechat_channels"]}));
  const result=await run(process.execPath,[path.join(V2_DIR,"run-platform.mjs"),"wechat_channels",packagePath,"publish","publish-auth","123"],{env:{...process.env,VIDEO_PUBLISHER_CONFIG:configPath}});
  assert.equal(result.code,2);
  assert.match(result.stderr,/requires --confirm-final-publish/);
});
