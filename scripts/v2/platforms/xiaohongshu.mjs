...
async function ensureXhsPrefillResponsive(){const attempts=[];for(let attempt=1;attempt<=2;attempt+=1){try{await cdp('Page.bringToFront',{}).catch(()=>{});await cdp('Page.setWebLifecycleState',{state:'active'}).catch(()=>{});await cdp('Emulation.setFocusEmulationEnabled',{enabled:true}).catch(()=>{});const state=await js(String.raw`(() => {const input=document.querySelector('input[placeholder*="填写标题"]');const editor=[...document.querySelectorAll('[contenteditable="true"],[contenteditable=""]')].find(el=>/话题|creator-editor/i.test(String(el.className||'')))||document.querySelector('[contenteditable="true"],[contenteditable=""]');const visible=el=>{if(!el)return false;const r=el.getBoundingClientRect(),s=getComputedStyle(el);return r.width>8&&r.height>8&&s.display!=='none'&&s.visibility!=='hidden'};return {ok:visible(input)&&visible(editor)&&document.visibilityState==='visible',visibility:document.visibilityState,focused:document.hasFocus()}})()`);if(state.ok)return {ok:true,attempt,attempts};attempts.push({attempt,state})}catch(error){attempts.push({attempt,reason:String(error?.message||error)})}if(attempt<2)await wait(.35)}return {ok:false,reason:'xiaohongshu title or editor is not responsive during upload',attempts}}
...
async function prefillXiaohongshu(){
  ...
  const actions={};actions.responsiveBeforeTitle=await ensureXhsPrefillResponsive();if(!actions.responsiveBeforeTitle.ok)return {...before,blocker:typedBlocker('INPUT_CHANNEL_BROKEN',actions.responsiveBeforeTitle.reason,{retryable:true,evidence:actions.responsiveBeforeTitle})};if(!before.gates.title.ok){actions.title=await setNativeInputValue('input[placeholder*="填写标题"]',xhsTitle);if(!actions.title.ok)return {...(await inspectXiaohongshu()),blocker:typedBlocker('ACTION_FAILED','小红书标题没有写入',{evidence:actions.title})}}
  const current=await inspectXiaohongshu();if(!current.gates.description.ok||!current.gates.tags.ok){actions.responsiveBeforeContent=await ensureXhsPrefillResponsive();if(!actions.responsiveBeforeContent.ok)return {...current,blocker:typedBlocker('INPUT_CHANNEL_BROKEN',actions.responsiveBeforeContent.reason,{retryable:true,evidence:actions.responsiveBeforeContent})};actions.content=await rebuildXhsTopics();if(!actions.content.ok)return {...(await inspectXiaohongshu()),blocker:typedBlocker('ACTION_FAILED',actions.content.reason,{evidence:actions.content})}}
...
async function mutateXiaohongshu() {
  ...
  const actions = {};
  actions.responsiveBeforeTitle=await ensureXhsPrefillResponsive();
  if(!actions.responsiveBeforeTitle.ok)return {...before,blocker:typedBlocker('INPUT_CHANNEL_BROKEN',actions.responsiveBeforeTitle.reason,{retryable:true,evidence:actions.responsiveBeforeTitle})};
  ...
  if (!afterTitle.gates.description.ok||!afterTitle.gates.tags.ok) {actions.responsiveBeforeContent=await ensureXhsPrefillResponsive();if(!actions.responsiveBeforeContent.ok)return {...afterTitle,blocker:typedBlocker('INPUT_CHANNEL_BROKEN',actions.responsiveBeforeContent.reason,{retryable:true,evidence:actions.responsiveBeforeContent})};actions.content = await rebuildXhsTopics();}
...
async function publishXiaohongshu(){
  const before=await inspectXiaohongshu();const required=publishProfile==='fast'?['authenticated','draftIdentity','video','title','description','tags','original','contentType','schedule','noBlockingDialog','finalButton']:Object.keys(before.gates);const missing=required.filter(name=>before.gates[name]?.ok!==true);if(missing.length)return {...before,blocker:typedBlocker('STATE_AMBIGUOUS','小红书没有通过发布前全部页面条件',{evidence:{missing}})};
  ...
  await click('@'+match[1],{label:'publish verified Xiaohongshu note'});let confirmationClicked=false;let semanticRetry=false;
  for(let attempt=0;attempt<30;attempt+=1){await wait(.5);const probe=await probeXhsPublishResult();if(probe.confirmed)return {...before,published:true,finalPublishClicked:true,publishReceipt:{confirmed:true,confirmationClicked,semanticRetry,signals:probe.signals,url:probe.url,at:new Date().toISOString()}};if(probe.errors.length)return {...before,finalPublishClicked:true,blocker:typedBlocker('ACTION_FAILED','小红书返回发布失败',{evidence:probe})};if(!confirmationClicked){const dialog=probe.dialogs.find(item=>/发布|提交/.test(item.text));const button=dialog?.buttons.find(item=>/^(确认|确定|发布|确认发布)$/.test(item.text)&&!item.disabled);if(button){await click([button.x,button.y],{label:'confirm Xiaohongshu publish'});confirmationClicked=true;continue}}
    if(!semanticRetry&&attempt===11&&!confirmationClicked&&probe.dialogs.length===0&&!probe.leftEditor){const retrySnapshot=await snapshotText();const retryMatch=String(retrySnapshot).match(/button \[ref=(\d+)[^\]]*\]\s*\n\s*text "(发布|定时发布)"/);if(retryMatch){await click('@'+retryMatch[1],{label:'retry unresponsive Xiaohongshu publish'});semanticRetry=true}}
  }
