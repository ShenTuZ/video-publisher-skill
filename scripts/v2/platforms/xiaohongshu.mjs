...
async function publishXiaohongshu(){
  const before=await inspectXiaohongshu();const required=publishProfile==='fast'?['authenticated','draftIdentity','video','title','description','tags','original','contentType','noBlockingDialog','finalButton']:Object.keys(before.gates);const missing=required.filter(name=>before.gates[name]?.ok!==true);if(missing.length)return {...before,blocker:typedBlocker('STATE_AMBIGUOUS','小红书没有通过发布前全部页面条件',{evidence:{missing}})};
