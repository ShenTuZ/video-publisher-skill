...
async function startWechatUpload(){
  ...
  if(!ready.ok){await gotoAndWait(PLATFORM_URLS.wechat_channels,{timeout:45,settle:2});ready=await waitWechatSdkReady(90)}
...
async function ensureWechatProductLink(currentGate=null){
  const gate=currentGate||(await inspectWechatChannels()).gates.productLink;
  if(gate.ok)return {ok:true,already:true};
  if(wechatLink.type!=='product')return {ok:false,reason:'wechat existing link cannot be safely cleared'};
  const target=await js(String.raw`(() => {const compact=v=>String(v||'').replace(/\s+/g,' ').trim();const roots=[document,...[...document.querySelectorAll('*')].map(el=>el.shadowRoot).filter(Boolean)];const section=roots.flatMap(root=>[...root.querySelectorAll('.form-item.post-with-link')])[0];if(!section)return {ok:false,reason:'wechat link control missing'};const pending=section.querySelector('.post-component-choose-wrap');const choose=[...section.querySelectorAll('div,span,button')].find(el=>compact(el.innerText||el.textContent||'')==='选择链接');const el=pending||choose;if(!el)return {ok:false,reason:'wechat link control missing'};el.scrollIntoView({block:'center'});const r=el.getBoundingClientRect();return {ok:true,mode:pending?'product_pending':'choose_type',x:r.left+r.width/2,y:r.top+r.height/2}})()`);
  if(!target.ok)return target;
  if(target.mode==='choose_type'){
    await click([target.x,target.y],{label:'open link choices'});
    await wait(1);
    const productOption=await js(String.raw`(() => {const compact=v=>String(v||'').replace(/\s+/g,' ').trim();const roots=[document,...[...document.querySelectorAll('*')].map(el=>el.shadowRoot).filter(Boolean)];const visible=el=>{const r=el.getBoundingClientRect(),s=getComputedStyle(el);return r.width>10&&r.height>10&&s.display!=='none'&&s.visibility!=='hidden'};const el=roots.flatMap(root=>[...root.querySelectorAll('.link-option-item,div,span,li')]).filter(visible).find(el=>compact(el.innerText||el.textContent||'')==='商品');if(!el)return {ok:false,reason:'wechat product link option unavailable for this page/account'};const r=el.getBoundingClientRect();return {ok:true,x:r.left+r.width/2,y:r.top+r.height/2}})()`);
    if(!productOption.ok)return productOption;
    await click([productOption.x,productOption.y],{label:'choose product link'});
    await wait(1);
  }
  const chooser=await js(String.raw`(() => {const roots=[document,...[...document.querySelectorAll('*')].map(el=>el.shadowRoot).filter(Boolean)];const el=roots.flatMap(root=>[...root.querySelectorAll('.form-item.post-with-link .post-component-choose-wrap')])[0];if(!el)return {ok:false,reason:'wechat product chooser missing'};el.scrollIntoView({block:'center'});const r=el.getBoundingClientRect();return {ok:true,x:r.left+r.width/2,y:r.top+r.height/2}})()`);
  if(!chooser.ok)return chooser;
  await click([chooser.x,chooser.y],{label:'open product chooser'});
  await wait(2);

  let candidate;
  if(wechatLink.selection==='first'){
    candidate=await js(String.raw`(() => {const compact=v=>String(v||'').replace(/\s+/g,' ').trim();const roots=[document,...[...document.querySelectorAll('*')].map(el=>el.shadowRoot).filter(Boolean)];const visible=el=>{const r=el.getBoundingClientRect(),s=getComputedStyle(el);return r.width>20&&r.height>20&&s.display!=='none'&&s.visibility!=='hidden'};const rows=roots.flatMap(root=>[...root.querySelectorAll('.ant-table-tbody tr.ant-table-row,tbody tr,[class*="product-item"],[class*="goods-item"]')]).filter(visible).filter(el=>{const text=compact(el.innerText||el.textContent||'');return text&&!/停业中|不可添加|已失效|暂无商品/.test(text)});const row=rows[0];if(!row)return {ok:false,reason:'wechat first selectable product missing'};const button=[...row.querySelectorAll('button,[role="button"],a')].find(el=>visible(el)&&/^(添加|选择)$/.test(compact(el.innerText||el.textContent||'')));const el=button||row;const r=el.getBoundingClientRect();return {ok:true,x:r.left+r.width/2,y:r.top+r.height/2,text:compact(row.innerText||row.textContent||'').slice(0,500)}})()`);
  }else{
    const search=await js(String.raw`(() => {const roots=[document,...[...document.querySelectorAll('*')].map(el=>el.shadowRoot).filter(Boolean)];const visible=el=>{const r=el.getBoundingClientRect(),s=getComputedStyle(el);return r.width>10&&r.height>10&&s.display!=='none'&&s.visibility!=='hidden'};const input=roots.flatMap(root=>[...root.querySelectorAll('input')]).filter(visible).find(el=>/商品名称|商品ID|关键词/.test(el.placeholder||''));if(!input)return {ok:false,reason:'wechat product search input missing'};input.focus();input.select();const r=input.getBoundingClientRect();return {ok:true,x:r.left+r.width/2,y:r.top+r.height/2}})()`);
    if(!search.ok)return search;
    await typeText(wechatLink.query);
    await wait(2);
    candidate=await js(String.raw`((needle) => {const compact=v=>String(v||'').replace(/\s+/g,' ').trim();const roots=[document,...[...document.querySelectorAll('*')].map(el=>el.shadowRoot).filter(Boolean)];const visible=el=>{const r=el.getBoundingClientRect(),s=getComputedStyle(el);return r.width>10&&r.height>10&&s.display!=='none'&&s.visibility!=='hidden'};const matches=roots.flatMap(root=>[...root.querySelectorAll('li,[role="option"],div')]).filter(visible).filter(el=>compact(el.innerText||el.textContent||'').includes(needle)).sort((a,b)=>compact(a.innerText||a.textContent||'').length-compact(b.innerText||b.textContent||'').length);const el=matches[0];if(!el)return {ok:false,reason:'wechat requested product not found'};const r=el.getBoundingClientRect();return {ok:true,x:r.left+r.width/2,y:r.top+r.height/2,text:compact(el.innerText||el.textContent||'')}})(${JSON.stringify(wechatLink.expectedName||wechatLink.query)})`);
  }
  if(!candidate.ok)return candidate;
  await click([candidate.x,candidate.y],{label:'select requested product'});
  await wait(1);
  const add=await js(String.raw`(() => {const compact=v=>String(v||'').replace(/\s+/g,' ').trim();const roots=[document,...[...document.querySelectorAll('*')].map(el=>el.shadowRoot).filter(Boolean)];const visible=el=>{const r=el.getBoundingClientRect(),s=getComputedStyle(el);return r.width>20&&r.height>20&&s.display!=='none'&&s.visibility!=='hidden'};const el=roots.flatMap(root=>[...root.querySelectorAll('button,[role="button"]')]).filter(visible).find(el=>/^添加\(\d+\)$/.test(compact(el.innerText||el.textContent||''))&&!el.disabled);if(!el)return {ok:false,reason:'wechat selected-product add button missing'};const r=el.getBoundingClientRect();return {ok:true,x:r.left+r.width/2,y:r.top+r.height/2,text:compact(el.innerText||el.textContent||'')}})()`);
  if(!add.ok)return add;
  await click([add.x,add.y],{label:'add selected product'});
  await wait(2);
  const timingConfirm=await js(String.raw`(() => {const compact=v=>String(v||'').replace(/\s+/g,' ').trim();const roots=[document,...[...document.querySelectorAll('*')].map(el=>el.shadowRoot).filter(Boolean)];const visible=el=>{const r=el.getBoundingClientRect(),s=getComputedStyle(el);return r.width>20&&r.height>20&&s.display!=='none'&&s.visibility!=='hidden'};const dialog=roots.flatMap(root=>[...root.querySelectorAll('.weui-desktop-dialog__wrp,[role="dialog"]')]).filter(visible).find(el=>/选择商品出现时机/.test(compact(el.innerText||el.textContent||'')));if(!dialog)return {ok:true,needed:false};const el=[...dialog.querySelectorAll('button,[role="button"]')].find(el=>compact(el.innerText||el.textContent||'')==='确认'&&!el.disabled);if(!el)return {ok:false,reason:'wechat product timing confirm missing'};const r=el.getBoundingClientRect();return {ok:true,needed:true,x:r.left+r.width/2,y:r.top+r.height/2}})()`);
  if(!timingConfirm.ok)return timingConfirm;
  if(timingConfirm.needed){await click([timingConfirm.x,timingConfirm.y],{label:'confirm product timing'});await wait(2)}
  const after=await inspectWechatChannels();
  return {ok:after.gates.productLink.ok,evidence:after.gates.productLink.evidence,candidate};
}
...
async function prefillWechatChannels(){
  ...
  if(!before.gates.draftIdentity.ok)return {...before,blocker:typedBlocker('FOREIGN_DRAFT','视频号当前编辑器属于其他视频草稿',{evidence:before.gates.draftIdentity.evidence})};
  if(!before.gates.noBlockingDialog.ok)return {...before,blocker:typedBlocker('STATE_AMBIGUOUS','视频号预填前存在阻塞弹窗',{retryable:true,evidence:before.gates.noBlockingDialog.evidence})};
  const actions={};
  ...
  actions.activity=await ensureWechatNoActivity(before.gates.activity);
...
async function mutateWechatChannels(){
  ...
  if(!before.gates.noBlockingDialog.ok)return {...before,blocker:typedBlocker('STATE_AMBIGUOUS','视频号存在未识别的阻塞弹窗',{retryable:true,evidence:before.gates.noBlockingDialog.evidence})};
  if(!before.gates.description.ok||!before.gates.shortTitle.ok){actions.textFields=await setWechatTextFields({description:!before.gates.description.ok,shortTitle:!before.gates.shortTitle.ok});if(!actions.textFields.ok)return {...(await inspectWechatChannels()),blocker:typedBlocker('ACTION_FAILED',actions.textFields.reason||'视频号描述或短标题没有写入',{evidence:actions.textFields})}}
  actions.activity=await ensureWechatNoActivity(before.gates.activity);
  if(!actions.activity.ok)return {...(await inspectWechatChannels()),blocker:typedBlocker('ACTION_FAILED',actions.activity.reason||'视频号活动状态没有设为不参与',{evidence:actions.activity})};
  actions.schedule=await ensureWechatSchedule(before.gates.schedule);
