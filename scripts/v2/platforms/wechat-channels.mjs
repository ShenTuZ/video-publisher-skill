...
async function ensureWechatProductLink(currentGate=null){
  ...
  const selected=wechatLink.selection==='first'?await js(String.raw`(() => {const roots=[document,...[...document.querySelectorAll('*')].map(el=>el.shadowRoot).filter(Boolean)];const radio=roots.flatMap(root=>[...root.querySelectorAll('input[type="radio"]')]).find(el=>el.value);if(!radio)return {ok:false};radio.click();return {ok:radio.checked===true,native:true}})()`):{ok:false};
