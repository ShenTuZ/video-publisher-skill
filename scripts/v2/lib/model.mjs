export const PLATFORMS = ["xiaohongshu", "douyin", "wechat_channels"];
...
const REQUIRED_GATES = Object.freeze({
  ...
  douyin: [
    "authenticated", "draftIdentity", "video", "title", "description", "tags",
    "settings", "defaults", "visibility", "download", "schedule", "cover", "noBlockingDialog", "finalButton", "safety",
  ],
  wechat_channels: [
    "authenticated", "draftIdentity", "video", "description", "shortTitle",
    "activity", "schedule", "aiLabel", "productLink", "original", "cover", "noBlockingDialog", "finalButton", "safety",
  ],
});
...
function missingGateBlocker(observation, missing) {
  ...
  if (gates.draftIdentity?.ok === false && gates.draftIdentity?.evidence?.foreign === true) {
    return { code: BLOCKER.FOREIGN_DRAFT, message: "当前编辑器属于其他视频草稿", retryable: false, requiresUser: false };
  }
...
export function classifyVerdict(verdict) {
  ...
  if (verdict.blocker?.code === BLOCKER.FOREIGN_DRAFT) return "blocked_foreign_draft";
