...
export const CONFIG_PLATFORMS = ["xiaohongshu", "douyin", "wechat_channels"];
...
export function defaultConfig() {
  return {
    ...
    platforms: {
      ...
      wechat_channels: {
        locationPolicy: "preserve",
        collectionPolicy: "leave_unselected",
        activityPolicy: "none",
      },
    },
    execution: {
...
export function normalizeConfig(raw = {}) {
  ...
  return {
    ...
    platforms: {
      ...
      wechat_channels: {
        locationPolicy: String(raw.platforms?.wechat_channels?.locationPolicy || fallback.platforms.wechat_channels.locationPolicy).trim(),
        collectionPolicy: String(raw.platforms?.wechat_channels?.collectionPolicy || fallback.platforms.wechat_channels.collectionPolicy).trim(),
        activityPolicy: String(raw.platforms?.wechat_channels?.activityPolicy || fallback.platforms.wechat_channels.activityPolicy).trim(),
      },
    },
    execution: {
...
export function validateConfig(config) {
  ...
  if (config.platforms.wechat_channels.locationPolicy !== "preserve") {
    errors.push("platforms.wechat_channels.locationPolicy must be preserve");
  }
  if (config.platforms.wechat_channels.collectionPolicy !== "leave_unselected") {
    errors.push("platforms.wechat_channels.collectionPolicy must be leave_unselected");
  }
  if (config.platforms.wechat_channels.activityPolicy !== "none") {
    errors.push("platforms.wechat_channels.activityPolicy must be none");
  }
