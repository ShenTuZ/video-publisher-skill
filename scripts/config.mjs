...
function usage() {
  return [
    ...
    "",
    "Repeatable options:",
    "  --available-platform <xiaohongshu|douyin|wechat_channels>",
    "  --platform <xiaohongshu|douyin|wechat_channels>  Default platform; must be available",
    "  --recurring-tag <tag>",
    "  --douyin-topic <topic>",
    "",
    "Other options:",
...
function parseOptions(argv) {
  const repeatable = new Set(["--available-platform", "--platform", "--recurring-tag", "--douyin-topic"]);
  const single = new Set(["--source-dir", "--locale", "--copy-style", "--originality-policy", "--check-concurrency", "--upload-concurrency"]);
  const boolean = new Set(["--auto-publish-on-ready", "--upload-existing-cover-by-default"]);
  const result = {};
  ...
  if (command === "status" || command === "show") {
    ...
    const config = createOnboardedConfig({
      ...
      execution: {
        ...
        autoPublishOnReady: options["--auto-publish-on-ready"] === true,
      },
      cover: {
