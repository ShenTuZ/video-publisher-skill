...
test("completed onboarding writes a valid private per-user configuration", async () => {
  ...
  const config = createOnboardedConfig({
    ...
    availablePlatforms: ["xiaohongshu", "douyin", "wechat_channels"],
    defaultPlatforms: ["douyin", "wechat_channels"],
  ...
  assert.deepEqual(ready.config.availablePlatforms, ["xiaohongshu", "douyin", "wechat_channels"]);
...
test("onboarding CLI persists repeated platform and topic options", async () => {
  ...
  const result = await run(process.execPath, [
    ...
    "--auto-publish-on-ready",
  ], { env: { ...process.env, VIDEO_PUBLISHER_CONFIG: configPath } });
  assert.equal(result.code, 0, `${result.stderr}\n${result.stdout}`);
  const output = JSON.parse(result.stdout);
  assert.equal(output.onboardingRequired, false);
  assert.deepEqual(output.config.availablePlatforms, ["xiaohongshu", "douyin", "wechat_channels"]);
  assert.deepEqual(output.config.defaultPlatforms, ["douyin", "wechat_channels"]);
  ...
  assert.equal(output.config.execution.autoPublishOnReady, true);
  await fs.promises.rm(root, { recursive: true, force: true });
});
...
test("schema 1 configurations conservatively migrate available platforms from old defaults", async () => {
  ...
  assert.deepEqual(migrated.config.availablePlatforms, ["douyin", "wechat_channels"]);
  assert.deepEqual(migrated.config.defaultPlatforms, ["douyin", "wechat_channels"]);
  await fs.promises.rm(root, { recursive: true, force: true });
});
...
test("removed Bilibili platform is rejected before onboarding can complete", async () => {
  const root = await fs.promises.mkdtemp(path.join(os.tmpdir(), "video-publisher-config-removed-platform-"));
  const configPath = path.join(root, "config.json");
  const result = await run(process.execPath, [
    SCRIPT_PATH,
    "onboard",
  ...
  assert.match(result.stdout, /unsupported available platform: bilibili/);
