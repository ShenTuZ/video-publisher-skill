...
Before upload input, the runner performs two read-only browser-health inspections for every selected platform. If the shared Ego channel then returns `INPUT_CHANNEL_BROKEN`, this entry detects that typed blocker from the current attempt's state history and retries the same stateful job after 5 seconds and 10 seconds. It never reinjects an active upload; after the second unsuccessful recovery it exits with the saved state and no unverified final publish.
