...
2. inject supported uploads and exit each injection runner after upload-start proof
...
On 2026-07-14, the production scheduler passed a real three-platform run with three parallel uploads, serialized mutation, persisted task-space/cover state, interruption recovery, and three parallel fresh verifiers all `READY` before the final publish controls.
...
Another 2026-07-15 regression proved that per-job locks were insufficient for two different jobs: simultaneous runs split platform ownership and one state was left `running`. The state-root publisher lock now rejects one contender before state/browser work while preserving the owner's three-platform parallel phases. The refused real job's prior `READY` state remained byte-identical, and both normal release and dead-PID global-lock recovery passed live testing.

A real process-group `SIGKILL` during three parallel uploads subsequently left all three lock levels plus job state behind. The ordinary same-job retry removed only locks whose PIDs were dead, reused all three numeric task spaces, and received `resume_existing` from every upload adapter. It completed all custom-cover and metadata gates, reached three-platform `READY`, and stayed no-op `READY` for three reruns.
...
A subsequent cold-start run with exact paths containing a space before their suffixes completed three parallel uploads and all custom-cover receipts. It exposed and repaired Douyin's first-topic shared-text-node shape, then reached three-platform `READY` and passed three inspect/verify-only reruns.
