...
The browser channel is shared across task spaces. If any runner reports `INPUT_CHANNEL_BROKEN`, treat it as a phase-wide circuit breaker: wait for all already-started parallel runners, skip every remaining upload/mutate action, and run only final read-only verification. The normal fast entry retries the same persisted job after 5 seconds and 10 seconds only when its latest state contains this exact typed blocker; it then stops rather than starting a third unbounded recovery.
