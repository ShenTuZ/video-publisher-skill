...
1. Ask which supported creator platforms the user actually has: Xiaohongshu, Douyin, and WeChat Channels. Require at least one; do not preselect all three.
...
5. Ask Douyin default topics only when Douyin is available.
6. Ask whether every video may truthfully be declared original only when Xiaohongshu is available. Keep `ask_each_run` unless the user explicitly confirms `all_videos_original`. WeChat originality is never stored as a standing default; it remains false unless the current video request explicitly enables it.
...
- WeChat Channels location, collection, and activity policies to remain `preserve`, `leave_unselected`, and `none` respectively.
